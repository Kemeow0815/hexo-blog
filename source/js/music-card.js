...
(function() {
  ...
  function getCache() {
    try {
      ...
      // 检查缓存是否过期或配置是否改变
      if (Date.now() - timestamp > config_now.meting.cacheDuration || 
  ...
  async function fetchPlaylist(config) {
    ...
    if (config.meting && config.meting.enable && config.meting.id) {
      ...
      try {
        ...
        console.warn('[MusicCard] Meting API failed, using local playlist:', error);
      }
    }
  ...
  class MusicCard {
    ...
    render() {
      ...
      if (savedVolume !== null) {
        ...
        this.container.querySelector('.volume-slider').value = parseInt(savedVolume);
      } else {
    ...
    bindEvents() {
      ...
      this.audio.addEventListener('loadedmetadata', () => {
        ...
        this.container.querySelector('.duration').textContent = formatTime(this.duration);
      });
      ...
      this.audio.addEventListener('seeked', () => {
        ...
        if (Math.abs(actualTime - expectedTime) > 2 && expectedTime > 2) {
          console.warn('[MusicCard] Audio may not support seeking. Expected:', formatTime(expectedTime), 'Actual:', formatTime(actualTime));
      ...
      // 拖动功能
      let startX, startPercent, hasDragged = false;
      ...
      const onDragEnd = (e) => {
        ...
        console.log('[MusicCard] Drag end, seek to:', (percent * 100).toFixed(1) + '%');
        this.seek(percent);
    ...
    seek(percent) {
      ...
      console.log('[MusicCard] Seeking to:', formatTime(newTime), '/', formatTime(duration));
    ...
    updateProgress() {
      ...
      this.updateProgressUI(percent);
      this.container.querySelector('.current-time').textContent = formatTime(currentTime);
    }

    updateProgressUI(percent) {
      this.container.querySelector('.progress-fill').style.width = `${percent * 100}%`;
      this.container.querySelector('.progress-thumb').style.left = `${percent * 100}%`;
    }
    ...
    showSeekWarning() {
      ...
      if (!warningEl) {
        ...
        warning.style.cssText = 'font-size: 11px; color: #ff9800; text-align: center; margin-top: 4px;';
  ...
  function initMusicCards() {
    const containers = document.querySelectorAll('.music-card-widget, [data-music-card]');
