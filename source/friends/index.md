---
menu_id: more
layout: page
title: 友链
banner: https://p.268682.xyz/pic?img=ua
banner_info:
  avatar: https://jsd.268682.xyz/gh/Kemeow0815/img@main/img/kemiao0815.webp
  title: 友人帐
  subtitle: 这里的每个站点，都耐人寻味……
---

{% sites api:https://jsd.268682.xyz/gh/Kemeow0815/friends@output/v2/data.json %}

{% folding child:codeblock color:yellow 友链访问须知 %}
<div style="padding: 1rem;">

## 安全提示

本站严格遵守国家法律法规，所有外链均出于学习交流目的而设。由于互联网环境复杂多变，友链站点均为第三方独立运营，本人无法实时监控其内容更新与安全状况。部分站点可能存在未备案、域名未实名等情况，**访问前请知悉潜在风险**。

---

### 我的审核措施

- 收录前对站点主题内容进行初步筛查
- 定期检查友链页面是否存在异常

### 你可能面临的风险

- 站点所有者变更链接指向
- 服务器遭受攻击导致内容被篡改
- 域名过期后被恶意抢注利用
- 文章更新后植入广告或钓鱼链接
- 隐私政策不完善导致信息泄露

---

**如遇问题请在此页留言**，一般会在 48 小时内处理。紧急情况可发邮件至：<span style="color: #e74c3c;">[mcy@kemiaosw.top](mailto:mcy@kemiaosw.top)</span>

</div>
{% endfolding %}

## 友链数据

想加入[**友链屋**](https://www.268682.xyz)的[点此](https://github.com/Kemeow0815/miaoluoge-links/issues/new?template=friend-link-request.yml)，想加入本站友链，[点此](https://github.com/Kemeow0815/friends)~

{% tabs active:1 align:center %}

<!-- tab 一键复制 -->
{% copy 喵洛阁 prefix:title %}
{% copy https://www.kemiaosw.top prefix:url %}
{% copy https://jsd.268682.xyz/gh/Kemeow0815/img@main/img/kemiao0815.webp prefix:avatar %}
{% copy https://imgbed.268682.xyz/v2/wFOi4UQ.png prefix:screenshot %}
{% copy 每一段旅行，都有终点。 prefix:description %}
{% copy https://kemeow0815.github.io/feed.xml prefix:feed %}

<!-- tab 格式化信息  -->
```yml
title: 喵洛阁
url: https://www.kemiaosw.top
avatar: https://jsd.268682.xyz/gh/Kemeow0815/img@main/img/kemiao0815.webp
screenshot: https://imgbed.268682.xyz/v2/wFOi4UQ.png
description: 每一段旅行，都有终点。
feed: https://blog.kemiaosw.top/feed.xml
```

<!-- tab 文本信息 -->
- 网站名称: 喵洛阁
- 网站地址: https://www.kemiaosw.top
- 头像: https://jsd.268682.xyz/gh/Kemeow0815/img@main/img/kemiao0815.webp
- 网站截图: https://imgbed.268682.xyz/v2/wFOi4UQ.png
- 网站描述: 每一段旅行，都有终点。
- 网站RSS: https://kemeow0815.github.io/feed.xml


{% endtabs %}