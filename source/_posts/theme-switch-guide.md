...
ai_summary: Hexo Stellar 博客主题的明暗模式切换功能通过导航栏上的三个按钮（月亮、太阳和AI图标）实现，用户可以通过点击这些按钮快速切换到不同的明暗模式。该功能不仅提供了优雅的主题切换体验，还支持即时反馈和持久化存储，使得每个域名独立存储，容量为5MB，足以满足大多数配置需求。此外，该主题还支持PJAX技术，实现了无刷新导航，进一步提升了用户体验。
---
